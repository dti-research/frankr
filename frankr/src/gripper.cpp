...
Gripper::Gripper(const std::string& fci_ip, double gripper_speed, double gripper_force): franka::Gripper(fci_ip), fci_ip(fci_ip), gripper_speed(gripper_speed), gripper_force(gripper_force) { }
...
bool Gripper::homing() const {
  try {
    ...
    this->stop();
    ((franka::Gripper*) this)->move(0.04, 0.01); // [m] [m/s]
    return ((franka::Gripper*) this)->homing();
  }
}
